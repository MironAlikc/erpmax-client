...
## Description
New rows are loaded when scrolling reaches the end or when you can no longer move from the last row with the keyboard arrow keys or the PageDown button. The example behaves similarly to pre-creating fake rows instead of fetching data from the server and processing them on the server.
...
## Source
Original file: `/tmp/pluto_grid_docs/demo/lib/screen/feature/row_infinity_scroll_screen.dart`
