# Add Rows Asynchronously.Dart

## Description
The grid can freeze if there are many rows at the start of the grid. When a row is first set up or a new row is added, the PlutoGridStateManager.initializeRows method is executed and the necessary settings are made for the grid. In the example, an empty row is provided at the start of the grid, and the row is set asynchronously and reflected in the grid. See the example code for details.
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        body: AddRowsAsynchronouslyScreen(),
      ),
    );
  }
}
...
## Source
Original file: `/tmp/pluto_grid_docs/demo/lib/screen/feature/add_rows_asynchronously.dart`
